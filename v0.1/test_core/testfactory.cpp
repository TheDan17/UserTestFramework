...
#include <algorithm>
#include <random>
std::unique_ptr<Test> TestFactory::createTest(std::weak_ptr<TestDataset> data, TestParams params){
    ...
    // CREATE TEST OBJECTS
    std::vector<std::unique_ptr<TestLine>> dataTest;
    std::shared_ptr<TestData> bufferData;
    for (auto& dataPtr : testContent){
        bufferData = dataPtr.lock();
        dataTest.push_back(std::make_unique<TestLine>(bufferData, dataHideIndexes));
    }
    return std::make_unique<Test>(std::move(dataTest));
}
